...
def run_kmeans(pose_origins, cluster_num):
    ...
    pose_clips = KMeans(n_clusters=best_cluster_num, random_state=2023, n_init=20, max_iter=500).fit_predict(pose_origins)
    return pose_clips
...
def run_SC(poses, pose_origins, cluster_num, use_se3):
    if use_se3:
        pose_clips = SpectralClustering(n_clusters=cluster_num, assign_labels="kmeans", random_state=2023, affinity=se3_affinity).fit_predict(poses.reshape(poses.shape[0], -1))
    else:
        pose_clips = SpectralClustering(n_clusters=cluster_num, assign_labels="kmeans", random_state=2023).fit_predict(pose_origins)
    return pose_clips
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_path", type=str, default="D:/workspace/NeRFRealData/17F_Test/20230607_dining_ip12_hloc_600/transforms.json", help="输入文件路径")
